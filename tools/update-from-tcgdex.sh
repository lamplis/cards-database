...
usage() {
	echo "Usage: tools/update-from-tcgdex.sh [--stash] [--dry-run] [--theirs] [--validate]"
	echo
	echo "  --stash     Stash dirty worktree automatically before syncing"
	echo "  --dry-run   Print commands without changing git state"
	echo "  --theirs    Resolve rebase conflicts preferring the replayed Cardium commits"
	echo "              (git rebase -X theirs upstream/master)"
	echo "  --validate  Run tools/validate-database.sh after a successful sync"
}
...
if [ "$USE_THEIRS" -eq 1 ]; then
	run git rebase -X theirs upstream/master
else
...
if [ "$DRY_RUN" -eq 1 ]; then
	echo "+ sh tools/check-thirdparty-ids.sh"
else
...
if [ "$RUN_VALIDATE" -eq 1 ]; then
	if [ "$DRY_RUN" -eq 1 ]; then
		echo "+ sh tools/validate-database.sh"
	else
		echo "[i] Running validation gate..."
		sh "$ROOT/tools/validate-database.sh"
	fi
fi
